...
_('.accordion-item').on('closed', function (e) {
	...
	$.getJSON('../../fonctions/prospet/affiche.php', function(json, textStatus) {
		...
		data=json[i];
			$("#liste_prospect").append('<li class="accordion-item"><a href="#" class="item-content item-link"><div class="item-inner"><div class="item-title">'+data.nom+'</div></div></a><div class="accordion-item-content"><div class="content-block"><div>id : '+data.id+'</div><div>nom : '+data.nom+'</div><div>raison social: '+data.raison_socialt+'</div><div>téléphone : '+data.tel+'</div><div>fax : '+data.fax+'</div><div>email : '+data.email+'</div><div>domaine : '+data.domaine+'</div><div onclick="modifier('+data.id+')" style="color:blue;cursor:pointer"><span  class="modifier" data-id="'+data.id+'">modifier</span></div></li>');};
	})
	});
</script>
</body>
