...
{
	myApp.confirm('voulez-vous supprimer ce appel?', 'Custom Title', function () {
...
_('.accordion-item').on('closed', function (e) {
	...
	$.getJSON('../../fonctions/appelles/affiche.php', function(json, textStatus) {
		for (var i = 0; i < json.length; i++) {
		
		data=json[i];
			$("#liste_appel").append('<li class="accordion-item"><a href="#" class="item-content item-link"><div class="item-inner"><div class="item-title">'+data.nom_contact+' date : '+data.date+'</div></div></a><div class="accordion-item-content"><div class="content-block"><div>id : '+data.id+'</div><div>nom : '+data.nom_contact+'</div><div>prospect: '+data.nom_prospect+'</div><div>date : '+data.date+'</div><div>resultat : '+data.resultat_libelle+'</div><div>commentaire : '+data.commentaire+'</div><div onclick="modifier('+data.id+')" style="color:blue;cursor:pointer"><span  class="modifier" data-id="'+data.id+'">modifier</span></div><div onclick="supprimer('+data.id+')" style="color:red;cursor:pointer"><span  class="modifier" data-id="'+data.id+'">Supprimer</span></div></li>');};
	})
	});
</script>
</body>
