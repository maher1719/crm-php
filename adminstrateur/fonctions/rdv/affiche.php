...
$appelles=new rdv();
$liste_rdv=$appelles->afficher_rdv('rdv.*,resultat.libelle, CONCAT(contact.prenom," ",contact.nom) as nom_contact, prospect.nom as nom_prospect,CONCAT(utilisateur.nom," ",utilisateur.prenom) as nom_utilisateur',' inner join utilisateur on rdv.user_id = utilisateur.id inner join resultat on rdv.resultat=resultat.id inner join prospect on prospect.id=rdv.id_prospect inner join contact on contact.id=rdv.id_contact '.$select);
